...
var component = decodeURI((RegExp('option' + '=' + '(.+?)(&|$)').exec(location.search)||[,null])[1]);
var view = decodeURI((RegExp('view' + '=' + '(.+?)(&|$)').exec(location.search)||[,null])[1]);

console.log("component = "+component + " and view = " + view);
...
jQuery( function(){
	...
	jQuery( '#file_upload' ).uploadify( {
		'buttonText': di.upload_text,
		'formData': {
		},
		'swf': di.url.root + 'media/com_di/js/uploadify/uploadify.swf',
		'uploader': di.url.ajax + '?option=com_di&task=ajax.upload&object_id=' + di.object_id + '&session_name=' + di.form_data.session_name + '&session_id=' + di.form_data.session_id+"&component="+component+"&view="+view,
		'method': 'get',
		'auto': true,
		'debug': false,
		'multi': true,
	...
	jQuery( '.resize-images' ).click( function( event ){
		...
		jQuery( '#diImageList .item' ).each( function(){
			_images.push( jQuery( this ).attr( 'data-object-image-id' ) );
		} );
			...
			success: function( response ){
				...
				if( featured_value == 1 ){
					if( parseInt( response.status ) > 0 && di.use_featured > 0 ){
						var src_thumb 	= di.url.media + '/' + item.attr( 'data-object-id' ) + '_' + item.attr( 'data-object-image-id' ) + '_thumb_' + item.attr( 'data-filename' );
						var src_regular = di.url.media + '/' + item.attr( 'data-object-id' ) + '_' + item.attr( 'data-object-image-id' ) + '_regular_' + item.attr( 'data-filename' );
			...
			success: function( response ){
				if( parseInt( response.status ) > 0 ){
					...
					if( featured == 1 ){
						...
						if( di.use_featured > 0 ){
							var src_thumb 	= di.url.media + '/' + element.attr( 'data-object-id' ) + '_' + element.attr( 'data-object-image-id' ) + '_thumb_' + element.attr( 'data-filename' )
							var src_regular = di.url.media + '/' + element.attr( 'data-object-id' ) + '_' + element.attr( 'data-object-image-id' ) + '_regular_' + element.attr( 'data-filename' )
